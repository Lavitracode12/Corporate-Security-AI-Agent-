...
# ──────────────────────────────────────────────────────────────────────────
# DESIGN SYSTEM — tokens, fonts, component theming
# Palette: near-black slate ground, amber (case / evidence marker) as the
# primary accent, cyan as the "data / vector" accent, sage + rust for
# safe / threat status. Display type: Space Grotesk. Body: IBM Plex Sans.
# Data / trace / timestamps: IBM Plex Mono — this is a forensics console,
# so anything that looks like "system output" is set in mono on purpose.
# ──────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
# st.set_page_config(page_title="Corporate Security Memory Agent UI", layout="wide")
...
#     st.subheader("📚 Enterprise Knowledge Fabrics")
#     st.success("📂 Forensics Log Ledger (SQLite online)")
#     st.success("📊 HR Performance Database (SQLite online)")
#     st.success("📜 Compliance Policy Matrix (SQLite online)")
    
#     st.markdown("---")

# st.title("🛡️ Corporate Security Memory Agent")
# st.subheader("Persistent Hybrid Graph-Vector Cross-Source Forensic Investigation Engine")
...
# if st.button("Execute Multi-Hop Forensics Routing Sequence"):
#     with st.spinner("Traversing across the documents..."):
#         try:
#             res = requests.post(
#                 "http://127.0.0.1:8080/query", 
...
#             if res.get("status") == "blocked" or "SECURITY CLEARANCE DENIED" in str(res.get("summary_verdict")):
#                 st.error("🛑 **Active Security Intercept: Adversarial Threat Neutralized**")
#                 st.warning(res.get("summary_verdict", "This directive violates established corporate safety guardrails."))
#                 st.stop()  
            
#             elif res.get("threat_level") == "N/A":
#                 # INTERACTIVE CHAT MODE VIEW 
#                 st.success("💬 Interactive Security Chat Mode Active")
...
#                 col1, col2 = st.columns(2)
#                 with col1:
#                     st.metric(label="Calculated Threat Score Indicator", value=f"{res.get('threat_score')}/100")
#                 with col2:
#                     st.metric(label="Threat Classification Level Status", value=res.get("threat_level"))
                    
#                 st.info(f"**Executive Forensic Verdict Summary:** {res.get('summary_verdict')}")
                
#                 # 2. Sequential Multihop Subquery Planner Processing Blocks Trace
#                 st.write("### Decomposed Investigative Search Steps Tracker (Multi-Hop Trace)")
#                 for i, step in enumerate(res.get("sub_queries", [])):
#                     st.write(f"**Step {i+1} Vector Node Traversal Path Flag:** `{step}`")
                
#                 # 3. Dynamic Chronological Log Timeline Data Grid Representation Matrix
#                 st.write("### Generated Unified Forensic Incident Chronological Timeline View")
#                 timeline_data = res.get("timeline", [])
                
#                 if not timeline_data:
#                     st.warning("No context records located in timeline payload matrix.")
#                 else:
#                     for event in timeline_data:
#                         with st.container():
#                             source_doc = event.get('source_document', event.get('source', 'LOG'))
#                             st.markdown(f"🔹 **[{event.get('timestamp', 'N/A')}]** | Source Channel: `{source_doc.upper()}`")
...
#                 if report_path:
#                     report_url = f"http://127.0.0.1:8080/report?path={report_path}"
#                     st.success("Professional Incident Audit Log PDF Compiled Successfully.")
#                     st.markdown(f"[📥 Download Official Incident Signature Report PDF]({report_url})")
                
#         except Exception as e:
#             st.error(f"Failed connection transaction stream intercept validation sequence checklist exception: {str(e)}")

# # BOTTOM SECTION: Interactive System Architecture Flowchart
# st.markdown("---")
# with st.expander("🔍 View System Graph Agent Architecture Diagram"):
#     try:
#         from Graph.workflow import app_graph
#         # Fetch the Mermaid PNG byte stream dynamically from your compiled graph object
#         graph_image_bytes = app_graph.get_graph().draw_mermaid_png()
#         st.image(graph_image_bytes, caption="Persistent Hybrid Forensic Engine State Machine Workflow Layout")
#     except Exception as e:
#         st.info("Architecture diagram rendering active. For full visualization, ensure graphviz is configured.")