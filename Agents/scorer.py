...
class ThreatScorer:
    ...
    def calculate_score(self, timeline, demo_mode: bool = False):
        """
        RAG Steps 2 & 3: Single-Prompt Reasoning Scorer with high-fidelity deterministic boundaries.
        """
        if not timeline:
        ...
        if self.client and not demo_mode:
            try:
                ...
                # Lock the temperature parameter so the model behaves deterministically
                response = self.client.models.generate_content(
                    model='gemini-2.5-flash',
                    contents=prompt,
                    config={"temperature": 0.0}  # Strictly 0.0 to prevent drifting scores on reload
                )
        ...
        # Safe stable fallback calculations
        base_score = 15.0
        risk_indicators = []
        ...
        if score > 45.0:
            ...
            verdict = (
                f"LOCAL AUDIT STATUS ({level}): Checked logs match typical corporate behaviors. No unified malicious signatures isolated."
            )
